...
const quoteHref = "/get-quote";

export function Header() {
  const [open, setOpen] = useState(false); const pathname = usePathname();
  const isActive = (href: string) => href === "/" ? pathname === "/" : !href.includes("#") && (pathname === href || pathname.startsWith(`${href}/`));
  return <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/90 shadow-[0_4px_20px_rgba(10,26,47,.04)] backdrop-blur-xl">
    <div className="container-site flex h-[4.5rem] items-center justify-between gap-6"><Logo />
      <nav aria-label="Main navigation" className="hidden items-center gap-7 lg:flex">{nav.map(([label, href]) => <Link key={href} href={href} aria-current={isActive(href) ? "page" : undefined} className={`relative py-3 text-sm font-semibold transition-colors after:absolute after:inset-x-0 after:bottom-0 after:h-0.5 after:origin-left after:scale-x-0 after:bg-blue-700 after:transition-transform hover:text-blue-700 hover:after:scale-x-100 ${isActive(href) ? "text-blue-700 after:scale-x-100" : "text-slate-700"}`}>{label}</Link>)}</nav>
