...
const nav = [["Home", "/"], ["Products", "/products"], ["Buyer Requests", "/buyer-requests"], ["Sourcing Service", "/sourcing-service"], ["Market Insights", "/market-insights"], ["About", "/about"], ["Contact", "/contact"]];
...
export function Header() {
  ...
  return <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/95 backdrop-blur">
    <div className="hidden bg-[#071a33] py-2 text-xs text-slate-300 lg:block"><div className="container-site flex justify-between"><span>Connecting trusted supply with global demand</span><div className="flex gap-5"><span>India · Global Markets</span><Link href="/commission-disclosure" className="hover:text-white">Transparent commission model</Link></div></div></div>
    <div className="container-site flex h-18 items-center justify-between gap-6"><Logo />
      ...
      <div className="hidden items-center gap-2 xl:flex"><Link href="/auth/login" className="inline-flex h-10 items-center gap-2 px-3 text-sm font-semibold text-slate-700"><UserRound size={17} /> Login</Link><Link href="/request" className="inline-flex h-10 items-center rounded-lg bg-blue-700 px-4 text-sm font-bold text-white transition hover:bg-blue-800">Submit a Requirement</Link></div>
